...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    if odoo.tools.config.get("track_row_count"):
    ...
    if odoo.tools.config.get("track_user_group"):
        # create a technical table
    ...
    if odoo.tools.config.get("track_row_count"):
        ...
        with registry.cursor() as cr:
            ...
            if bool(result):
                ...
                for r in result:
                    msg_all += msg(
                        r["table_name"], r["old_row_count"], r["new_row_count"]
                    )
                _logger.warning(
            ...
            if openupgrade.table_exists(cr, "openupgrade_track_user_group"):
                ...
                if bool(old_groups) and bool(new_groups):
                    ...
                    for old_data in old_groups:
                        new_data = [
                            d for d in new_groups if d["uid"] == old_data["uid"]
                        ]
                        if not new_data:
                            msg_all += msg(
                                str(old_data["uid"]),
                        ...
                        added_grps = list(
                            set(new_data[0]["gids"]) - set(old_data["gids"])
                        )
